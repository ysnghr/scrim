...
test("detokenize: allows untouched tool_input when there are no tokens", () => {
    ...
    const r = detokenize({ tool_name: "Write", tool_input: { file_path: "x.txt", content: "hello world" } }, vault);
...
test("detokenize: rewrites string leaves and returns updatedInput", () => {
    const vault = { resolve: (t) => (t === "⟦scrim:db_password:aaaa1111⟧" ? "hunter2" : null) };
    const r = detokenize({
        tool_name: "Write",
        tool_input: {
    ...
    const updated = r.output.hookSpecificOutput.updatedInput;
    assert.equal(updated.content, "password: hunter2");
});
test("detokenize: walks nested arrays (MultiEdit edits[])", () => {
    const vault = { resolve: (t) => (t === "⟦scrim:secrets:aaaa1111⟧" ? "AKIA-real" : null) };
    const r = detokenize({
        tool_name: "MultiEdit",
        tool_input: {
            file_path: "x.yml",
...
test("detokenize: fail-closed when any token cannot be resolved", () => {
    const vault = { resolve: () => null };
    const r = detokenize({ tool_name: "Write", tool_input: { file_path: "x", content: "x ⟦scrim:secrets:deadbeef⟧" } }, vault);
