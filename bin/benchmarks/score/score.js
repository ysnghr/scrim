...
// the wrong bytes.
function classFamily(klass) {
    if (klass === "secrets")
        return "secrets";
    if (klass === "internal_hostnames")
        return "host";
    if (klass.startsWith("pii_"))
        return "pii";
    return "other";
}
function classMatches(detClass, labelClass) {
    if (detClass === labelClass)
        return true;
    // Generic pii_customer detection against a specific pii_email label counts.
    if (classFamily(detClass) === "pii" && classFamily(labelClass) === "pii")
        return true;
    return false;
}
...
export function scoreRunner(runner, labels) {
    ...
    for (const f of idx.values())
        lookalikeAvailable += f.benign.length;
    // Stable order so the "first overlapping" tiebreak is reproducible.
    const dets = [...runner.detections].sort((a, b) => a.file.localeCompare(b.file) || a.start - b.start || a.end - b.end);
    for (const d of dets) {
        ...
        if (claimedLabel) {
            ...
            tp[c] = (tp[c] ?? 0) + 1;
            tpTotal++;
            if (exactMatch(d, claimedLabel))
                exact++;
            else if (iou(d, claimedLabel) >= 0.5)
                partial++;
            else
                partial++; // any overlap counts; we already required overlap > 0
            continue;
        }
        ...
        if (benign) {
            lookalikeMatches++;
            const c = CLASSES_TO_REPORT.includes(benign.class) ? benign.class : "secrets";
            fp[c] = (fp[c] ?? 0) + 1;
        }
        else {
...
function metricFrom(klass, tp, fp, fn) {
