...
export function runPerf(opts) {
    ...
    for (const b of buffers)
        totalBytes += Buffer.byteLength(b.text, "utf8");
    ...
    const tokens = [];
    for (let i = 0; i < 6; i++)
        tokens.push(v.tokenize(`hook-bench-${i}`, "secrets", "bench-rule"));
    const lines = [];
    for (let i = 0; i < 200; i++) {
...
function generateBlob(targetBytes) {
