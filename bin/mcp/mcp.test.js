...
import { safeRead, safeGrep, safeShell, safeWriteToken, scrimStatus, scrimDoctor, REQUIRED_DENY_RULES, } from "./tools.js";
...
test("safe_write_token: rejects an unknown token", () => {
    const root = freshRepo();
    const ctx = buildContext(root);
    assert.throws(() => safeWriteToken({ token: "⟦scrim:unknown:deadbeef⟧", newValue: "x" }, ctx), /unknown token/);
});
...
test("scrim_doctor: deny-rules-present passes when all required rules are set", () => {
    ...
    writeFileSync(join(root, ".claude", "settings.json"), JSON.stringify({ permissions: { deny: REQUIRED_DENY_RULES } }));
    const ctx = buildContext(root);
    const report = scrimDoctor(ctx);
    const denyCheck = report.checks.find((c) => c.name === "deny-rules-present");
...
test("scrim_doctor: deny-rules-present reports which rules are missing", () => {
    ...
    writeFileSync(join(root, ".claude", "settings.json"), JSON.stringify({ permissions: { deny: REQUIRED_DENY_RULES.slice(0, 3) } }));
    const ctx = buildContext(root);
    const report = scrimDoctor(ctx);
    const denyCheck = report.checks.find((c) => c.name === "deny-rules-present");
    assert.equal(denyCheck.status, "fail");
...
test("scrim_doctor: vault-healthy warns when near cap", () => {
    ...
    const ctx = buildContext(root);
    for (let i = 0; i < 9; i++)
        ctx.vault.tokenize(`v-${i}`, "secrets");
    const report = scrimDoctor(ctx);
