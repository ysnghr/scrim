...
import { loadPolicy, loadPolicyFromString, defaultPolicy, toEngineInput, actionFor, PolicyError, } from "./index.js";
function freshRepo() {
...
tune:
  custom_patterns:
    - name: bad
      ...
      class: pii_customer
`;
    assert.throws(() => loadPolicyFromString(yaml, "<test>"), /policy\.tune\.custom_patterns\[0\]\.regex.*invalid regex/);
});
test("wrong types are rejected with the field path", () => {
    assert.throws(() => loadPolicyFromString("fail_closed: notabool", "<test>"), /policy\.fail_closed.*expected boolean/);
    assert.throws(() => loadPolicyFromString("detection: yes", "<test>"), /policy\.detection.*expected an object/);
});
...
test("rejects unsupported version", () => {
    assert.throws(() => loadPolicyFromString("version: 2", "<test>"), /version.*only version 1/);
});
