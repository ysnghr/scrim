...
test("killer scenario: safe_read tokenizes, detokenize hook restores byte-for-byte", async () => {
    ...
    const readResult = (await callTool(mcp, "safe_read", { path: "config.yml" }));
    ...
    const hookOut = JSON.parse(hookRes.stdout);
...
test("plugin manifests reference valid paths", () => {
    const mcpJson = JSON.parse(readFileSync(join(PLUGIN_ROOT, ".mcp.json"), "utf8"));
