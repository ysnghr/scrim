...
export function encrypt(key: Buffer, plaintext: string): Buffer {
  if (key.length !== KEY_LEN) throw new Error("scrim: vault key must be 32 bytes");
...
export function decrypt(key: Buffer, packed: Buffer): string {
  if (key.length !== KEY_LEN) throw new Error("scrim: vault key must be 32 bytes");
  if (packed.length < IV_LEN + TAG_LEN) throw new Error("scrim: vault payload truncated");
