...
export interface DetokenizeStats {
  replaced: number;
  missing: string[];
}
...
export function detokenize(input: HookInput, vault: VaultLike): DetokenizeResult {
  const stats: DetokenizeStats = { replaced: 0, missing: [] };
      ...
      output: {
        hookSpecificOutput: {
          ...
          permissionDecisionReason:
            `scrim: refusing write — ${stats.missing.length} token(s) could not be resolved` +
