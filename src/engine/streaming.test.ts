...
function freshFile(content: string | Buffer): { path: string; cleanup: () => void } {
...
test("streaming: line numbers reflect file position", () => {
  ...
  const lines: string[] = [];
  for (let i = 0; i < 5; i++) lines.push(`# line ${i + 1}`);
  lines.push("TOKEN=AKIAIOSFODNN7EXAMPL2");
  for (let i = 0; i < 5; i++) lines.push(`# trailing ${i}`);
...
test("streaming: rejects overlap >= chunkBytes", () => {
  ...
  try {
    assert.throws(
      () => detectStreaming(path, cfg(), { chunkBytes: 1024, overlapBytes: 1024 }),
