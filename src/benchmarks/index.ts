...
import type { Detection, Label, RunnerResult, RunnerScore } from "./types.js";
...
export function runBench(opts: BenchOptions = {}): BenchOutcome {
  const root = repoRoot();
  ...
  if (enabled.has("scrim")) results.push(runScrim({ corpusDir, files }));
  if (enabled.has("gitleaks")) results.push(runGitleaks({ corpusDir, files }));
  if (enabled.has("trufflehog")) results.push(runTrufflehog({ corpusDir, files }));
  if (enabled.has("presidio")) results.push(runPresidio({ corpusDir, files }));
  if (enabled.has("naive-regex")) results.push(runNaive({ files }));
...
function writeDetections(outDir: string, runner: string, dets: Detection[]): void {
