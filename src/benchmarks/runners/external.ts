...
function totalBytes(files: { absPath: string }[]): number {
  let n = 0;
  for (const f of files) n += statSync(f.absPath).size;
  return n;
}

function rel(corpusDir: string, p: string): string {
  if (!p) return p;
  if (isAbsolute(p)) return relative(corpusDir, p);
...
// --- gitleaks ---
//
// `gitleaks detect --source <dir> --no-git --report-format json -r <out>` writes
// a JSON array of findings. Each has File, StartLine, EndLine, StartColumn,
// EndColumn, Match, RuleID. We don't try to convert (line,col) → byte offset
// faithfully — instead we re-search the matched substring in the file to
// recover the byte range. That's robust against CRLF and tab-vs-space.
...
export function runGitleaks(opts: GitleaksOptions): RunnerResult {
  ...
  try {
    ...
    const arr = JSON.parse(raw) as Array<{
      File?: string; StartLine?: number; Match?: string; Secret?: string; RuleID?: string;
    }>;
    for (const f of arr) {
      const file = rel(opts.corpusDir, f.File ?? "");
      const needle = f.Secret ?? f.Match;
      if (!needle || !file) continue;
    ...
    notes.push(`failed to parse gitleaks report: ${(e as Error).message}`);
  }
...
function normalizeGitleaksClass(ruleId: string): CorpusClass {
  ...
  if (ruleId === "email" || ruleId === "generic-email") return "pii_email";
  return "secrets";
}

// --- trufflehog ---
//
// `trufflehog filesystem <dir> --json` streams one JSON object per line. Each
// has SourceMetadata.Data.Filesystem.file, Raw, DetectorName. Trufflehog only
// finds secrets, so everything maps to "secrets".
...
export function runTrufflehog(opts: TrufflehogOptions): RunnerResult {
  ...
  for (const line of (r.stdout ?? "").split("\n")) {
    ...
    const raw: string | undefined = obj?.Raw;
...
// --- presidio ---
//
// Presidio runs as a Python library. We expect a thin wrapper script on PATH
// named `presidio-analyze` that accepts a file path and prints
// {file,start,end,entity_type,score}\n per finding. Most projects will have
// such a script; we don't try to inline a Python interpreter call here.

export interface PresidioOptions {
  corpusDir: string;
  files: { relPath: string; absPath: string }[];
  command?: string;     // default: "presidio-analyze"
}

export function runPresidio(opts: PresidioOptions): RunnerResult {
...
function normalizePresidioEntity(entity: string): CorpusClass {
  switch (entity) {
