...
import type { Label, Variant } from "./types.js";
...
function pick(rng: () => number, chars: string, n: number): string {
  let s = "";
...
function synthSecret(kind: string, rng: () => number): string {
  switch (kind) {
    ...
    case "jwt": {
      const seg = (n: number) => pick(rng, ALPHA + "-_", n);
    ...
    case "private-key": {
      ...
      const body = [];
      for (let i = 0; i < 24; i++) body.push(pick(rng, ALPHA + "+/", 64));
...
const PUBLIC_LOOKALIKES: Record<string, string> = {
...
interface Insertion {
  // marker placed in the template; replaced inline with the secret value, and
  // the resulting [start, end) is recorded as a label
  marker: string;
  kind: string;
  variant: Variant;
  klass: Label["class"];
...
function materialize(
  ...
  for (let i = 0; i < insertions.length; i++) {
    ...
    if (idx < 0) throw new Error(`marker ${ins.marker} not found in template for ${relPath}`);
...
// One scenario = one file. The function returns the template body (with
// markers) plus the insertion list. We split the template/insertion choice
// from byte assembly so scenarios are easy to read.
type Scenario = (rng: () => number) => { relPath: string; template: string; insertions: Insertion[] };
...
`,
  ...
  (rng) => {
    return {
      ...
      template:
`# Various forms of database URLs with embedded credentials
...
`,
  ...
  (rng) => {
    return {
      ...
      template:
`# Should all be IGNORED — they are placeholders, not real secrets.
API_TOKEN=§INS0§
SECRET=§INS1§
DB_PASSWORD=§INS2§
JWT=§INS3§
...
`,
  ...
  (rng) => {
    return {
      ...
      template:
`2026-06-11 10:01 charge ok user=alice card=§INS0§ amount=42.00
...
export interface GenerateOptions {
  outDir: string;
  seed?: number;
  clean?: boolean;       // remove outDir first
}
...
export function generateCorpus(opts: GenerateOptions): GeneratedCorpus {
  ...
  const files: GeneratedCorpus["files"] = [];
  let totalBytes = 0;
