...
export type SurvivConfig = "scrim" | "redact-only" | "no-redactor" | "permission-only";

export interface SurvivScenario {
  name: string;
  // Original file contents (real secrets included).
  originalContent: string;
  // Scripted transformation the agent performs on whatever text it received.
  // Receives the (possibly tokenized) text and returns the new text. Must be
  // a pure-text transformation — no knowledge of which bytes are tokens.
  edit: (text: string) => string;
  // Expected file contents after the edit, evaluated against the ORIGINAL
  // (real-secret) file so the oracle is independent of any redactor.
  expectedAfterEdit: string;
  // The raw secret values that exist in the original. Used to detect leaks.
  rawSecrets: string[];
}

export interface SurvivRow {
  scenario: string;
  config: SurvivConfig;
  completed: boolean;     // (a) agent's edit ran end-to-end and a file was written
  byteCorrect: boolean;   // (b) final on-disk file matches expectedAfterEdit
  noLeak: boolean;        // (c) the text the agent operated on did not contain any rawSecret
  reason?: string;
}

export interface SurvivabilityResult {
  rows: SurvivRow[];
  byConfig: { config: SurvivConfig; total: number; allThree: number }[];
}
...
const SCENARIOS: SurvivScenario[] = [
  (() => {
    const original =
`# production config
...
function runOne(scenario: SurvivScenario, config: SurvivConfig): SurvivRow {
  const repo = freshRepo();
  const filePath = join(repo, "target.txt");
  writeFileSync(filePath, scenario.originalContent);
  const row: SurvivRow = { scenario: scenario.name, config, completed: false, byteCorrect: false, noLeak: false };
  ...
  try {
    ...
    const finalText = ((hookOut.hookSpecificOutput.updatedInput as any)?.content ?? edited) as string;
    writeFileSync(filePath, finalText);
    row.completed = true;
    row.byteCorrect = readFileSync(filePath, "utf8") === scenario.expectedAfterEdit;
    return row;
  } catch (e) {
    row.reason = (e as Error).message;
    return row;
  }
}

export function runSurvivability(): SurvivabilityResult {
  const rows: SurvivRow[] = [];
  const configs: SurvivConfig[] = ["scrim", "redact-only", "no-redactor", "permission-only"];
  for (const s of SCENARIOS) {
    for (const c of configs) rows.push(runOne(s, c));
  }
